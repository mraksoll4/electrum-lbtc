...
SYSTEM_CONFIG_PATH = "/etc/electrum-lbtc.conf"
...
class SimpleConfig(PrintError):
    ...
    def get_wallet_path(self):
        ...
        old_path = os.path.join(self.path, "electrum-lbtc.dat")
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/electrum-lbtc.conf."""
    result = {}
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-lbtc.conf into user_config[]."""
    if not path:
        return {}
