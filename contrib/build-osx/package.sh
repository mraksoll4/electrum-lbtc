...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum-LBTC.app"
    exit -127
fi
...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-lbtc-macos
	cd /tmp/electrum-lbtc-macos
...
rm -rf /tmp/electrum-lbtc-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-lbtc-macos/image/
cp -r $1 /tmp/electrum-lbtc-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-LBTC_uncompressed.dmg \
    /tmp/electrum-lbtc-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-LBTC_uncompressed.dmg electrum-lbtc-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-LBTC_uncompressed.dmg

echo "Done."
md5sum electrum-lbtc-$VERSION.dmg
