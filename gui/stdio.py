...
from electrum_lbtc.util import format_satoshis, set_verbosity
from electrum_lbtc.bitcoin import is_address, COIN, TYPE_ADDRESS
...
class ElectrumGui:
    ...
    def __init__(self, config, daemon, plugins):
        ...
        if not storage.file_exists:
            print("Wallet not found. try 'electrum-lbtc create'")
            exit()
    ...
    def do_send(self):
        if not is_address(self.str_recipient):
            print(_('Invalid Litebitcoin address'))
            return
        try:
    ...
    def network_dialog(self):
        print("use 'electrum-lbtc setconfig server/proxy' to change your network settings")
        return True


    def settings_dialog(self):
        print("use 'electrum-lbtc setconfig' to change your settings")
        return True
