...
import electrum_lbtc as electrum
from electrum_lbtc.bitcoin import TYPE_ADDRESS
from electrum_lbtc import WalletStorage, Wallet
from electrum_lbtc_gui.kivy.i18n import _
from electrum_lbtc.paymentrequest import InvoiceStore
from electrum_lbtc.util import profiler, InvalidPassword
from electrum_lbtc.plugins import run_hook
from electrum_lbtc.util import format_satoshis, format_satoshis_plain
from electrum_lbtc.paymentrequest import PR_UNPAID, PR_PAID, PR_UNKNOWN, PR_EXPIRED
...
#Factory.register('InstallWizard', module='electrum_lbtc_gui.kivy.uix.dialogs.installwizard')
#Factory.register('InfoBubble', module='electrum_lbtc_gui.kivy.uix.dialogs')
#Factory.register('OutputList', module='electrum_lbtc_gui.kivy.uix.dialogs')
#Factory.register('OutputItem', module='electrum_lbtc_gui.kivy.uix.dialogs')
...
# the data
Cache.register('electrum_lbtc_widgets', timeout=0)
...
Factory.register('TabbedCarousel', module='electrum_lbtc_gui.kivy.uix.screens')
...
from electrum_lbtc.util import base_units
...
class ElectrumWindow(App):
    ...
    def choose_server_dialog(self, popup):
        ...
        protocol = 's'
        def cb2(host):
            from electrum_lbtc.bitcoin import NetworkConstants
    ...
    def on_new_intent(self, intent):
        if intent.getScheme() != 'litebitcoin':
            return
    ...
    def _get_bu(self):
        return self.electrum_config.get('base_unit', 'LBTC')
    ...
    def on_qr(self, data):
        from electrum_lbtc.bitcoin import base_decode, is_address
        data = data.strip()
        if is_address(data):
            self.set_URI(data)
            return
        if data.startswith('litebitcoin:'):
            self.set_URI(data)
            return
        # try to decode transaction
        from electrum_lbtc.transaction import Transaction
        from electrum_lbtc.util import bh2u
        try:
    ...
    def show_pr_details(self, req, status, is_invoice):
        from electrum_lbtc.util import format_time
    ...
    def init_ui(self):
        ...
        Factory.register('AnimatedPopup',
                         module='electrum_lbtc_gui.kivy.uix.dialogs')
        Factory.register('QRCodeWidget',
                         module='electrum_lbtc_gui.kivy.uix.qrcodewidget')

        # preload widgets. Remove this if you want to load the widgets on demand
        #Cache.append('electrum_lbtc_widgets', 'AnimatedPopup', Factory.AnimatedPopup())
        #Cache.append('electrum_lbtc_widgets', 'QRCodeWidget', Factory.QRCodeWidget())
        ...
        self.icon = "icons/electrum-lbtc.png"
    ...
    def notify(self, message):
        try:
            ...
            icon = (os.path.dirname(os.path.realpath(__file__))
                    + '/../../' + self.icon)
            notification.notify('Electrum-LBTC', message,
                            app_icon=icon, app_name='Electrum-LBTC')
        except ImportError:
